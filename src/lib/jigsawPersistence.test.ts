...
describe("jigsawPersistence", () => {
  ...
  test("restores a valid save with elapsed time, groups, coordinates and zoom", () => {
    const pieces = basePieces.map((piece) => ({ ...piece, pos: { x: piece.correct.x + 12, y: piece.correct.y + 8 } }));
    const save = serializeJigsawProgress({ signature: "sig", pieces, tray: ["0-0", "0-1", "1-0", "1-1"], elapsedMs: 12_345, zoom: 1.5, savedAt: 100 });
    ...
    expect(result?.progress.zoom).toBe(1.5);
    expect(result?.pieces[0].pos).toEqual({ x: 22, y: 18 });
  });

  test("same-day Daily restores but another day, Catalog and Warz cannot seed it", () => {
    const save = serializeJigsawProgress({ signature: "sig", pieces: basePieces, tray: basePieces.map((piece) => piece.id), elapsedMs: 10, zoom: 1 });
  ...
  test("migrates the old key only for Catalog", () => {
    const legacy = serializeJigsawProgress({ signature: "old", pieces: basePieces, tray: basePieces.map((piece) => piece.id), elapsedMs: 20, zoom: 1 });
  ...
  test.each([
    ...
    const save = serializeJigsawProgress({ signature: "sig", pieces: basePieces, tray: basePieces.map((piece) => piece.id), elapsedMs: 0, zoom: 1 });
