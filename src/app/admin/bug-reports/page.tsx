...
interface Report {
  ...
  puzzle: { id: string; title: string } | null;
}
...
export default function AdminBugReportsPage() {
  ...
  async function loadReports(tab: typeof STATUS_TABS[number]) {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/bug-reports?status=${tab}`);
      if (res.ok) {
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#020202" }}>
      ...
      <main className="max-w-5xl mx-auto px-4 pt-24 pb-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-white">Bug Reports</h1>
        </div>
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="space-y-1.5 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      {report.puzzle ? (
                        <Link
                          href={`/puzzles/${report.puzzle.id}`}
                          target="_blank"
                          className="text-xs font-bold px-2 py-0.5 rounded-full hover:underline"
                          style={{ background: "rgba(56,145,166,0.15)", color: "#3891A6", border: "1px solid rgba(56,145,166,0.3)" }}
                        >
                          🧩 {report.puzzle.title}
                        </Link>
                      ) : report.puzzleTitle ? (
                        <span
                          ...
                          title="This puzzle no longer exists"
                        >
                          🧩 {report.puzzleTitle} (deleted)
                        </span>
                      ) : (
                        <span
                          className="text-xs font-bold px-2 py-0.5 rounded-full"
                        ...
                        >
                          📍 {report.otherLocation || "Unspecified"}
                        </span>
                      )}
                    ...
                    <p className="text-sm text-white">
                      {report.user ? (
                        <Link href={`/profile/${report.user.id}`} className="font-semibold hover:underline" style={{ color: "#3891A6" }}>
                          {report.user.name ?? report.user.email ?? "Unknown"}
                        </Link>
                      ) : (
                        <span className="font-semibold" style={{ color: "#9CA3AF" }}>Anonymous</span>
                      )}
                      {report.contactEmail && (
                        <span className="text-xs ml-2" style={{ color: "#6B7280" }}>({report.contactEmail})</span>
                      )}
                    </p>
