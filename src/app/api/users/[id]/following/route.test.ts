...
describe("GET /api/users/[id]/following", () => {
  ...
  test("unexpected hidden row is dropped as defense in depth", async () => {
    mockedGetServerSession.mockResolvedValueOnce(null);
    mockedPrisma.follow.findMany.mockResolvedValueOnce([
      followingRow({ id: "follow-row-1", following: { id: "following-1", name: "Visible", image: null } }),
      followingRow({
        id: "follow-row-2",
        following: { id: "following-2", name: "Hidden", image: null, isHidden: true },
      }),
    ]);
    ...
    expect(body.users.map((u: { id: string }) => u.id)).toEqual(["following-1"]);
  });
