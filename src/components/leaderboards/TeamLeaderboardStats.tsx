...
export interface TeamLeaderboardStatsProps {
...
export default function TeamLeaderboardStats({ entries }: TeamLeaderboardStatsProps) {
  if (entries.length === 0) return null;

  const rankedTeams = entries.length;
  const totalPoints = entries.reduce((sum, entry) => sum + entry.totalPoints, 0);
  const puzzlesSolved = entries.reduce((sum, entry) => sum + entry.totalPuzzlesSolved, 0);

  return (
    <div data-testid="team-leaderboard-stats" className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
