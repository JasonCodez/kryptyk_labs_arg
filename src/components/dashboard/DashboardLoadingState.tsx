...
/**
 * Static skeleton mirroring the redesigned dashboard's layout: command
 * header, featured mission, 2x2 stat strip, and two navigation-group panels.
 * Deliberately motionless — no shimmer, pulse, or keyframes.
 */
export default function DashboardLoadingState() {
  return (
    <DashboardPageShell busy>
      ...
      <div aria-hidden="true" data-testid="skeleton-command-header" style={{ ...PANEL_STYLE, height: 148, marginBottom: 48 }} />

      {/* Featured Mission panel */}
      <div aria-hidden="true" data-testid="skeleton-featured-mission" style={{ ...PANEL_STYLE, height: 112, marginBottom: 40 }} />

      {/* Four-stat 2x2 area */}
      <div aria-hidden="true" data-testid="skeleton-stats" className="grid grid-cols-2 gap-3" style={{ marginBottom: 48 }}>
        {[0, 1, 2, 3].map((i) => (
          <div key={i} style={{ ...PANEL_STYLE, height: 76 }} />
        ))}
      </div>
      ...
      <div aria-hidden="true" data-testid="skeleton-navigation" className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {[0, 1].map((i) => (
          <div key={i} style={{ ...PANEL_STYLE, height: 180 }} />
        ))}
      </div>
    </DashboardPageShell>
  );
